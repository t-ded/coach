...
class TestStravaWebhookEvent:
    ...
    def test_deauthorization_event_calls_deauthorize(self) -> None:
        with patch('coach.web.strava_webhook.deauthorize_athlete') as mock_deauth:
            response = _post(self._http, {'object_type': 'athlete', 'aspect_type': 'deauthorization', 'owner_id': 42, 'object_id': 42})
    ...
    def test_activity_create_event_dispatches_handler(self) -> None:
        with patch('coach.web.strava_webhook._handle_activity_event') as mock_handler:
            response = _post(self._http, {'object_type': 'activity', 'aspect_type': 'create', 'owner_id': 42, 'object_id': 999})
    ...
    def test_activity_update_event_dispatches_handler(self) -> None:
        with patch('coach.web.strava_webhook._handle_activity_event') as mock_handler:
            response = _post(self._http, {'object_type': 'activity', 'aspect_type': 'update', 'owner_id': 42, 'object_id': 999})
    ...
    def test_activity_delete_event_dispatches_handler(self) -> None:
        with patch('coach.web.strava_webhook._handle_activity_event') as mock_handler:
            response = _post(self._http, {'object_type': 'activity', 'aspect_type': 'delete', 'owner_id': 42, 'object_id': 999})
    ...
    def test_unrecognised_event_returns_200(self) -> None:
        response = _post(self._http, {'object_type': 'something_new', 'aspect_type': 'unknown'})

        assert response.status_code == 200

    def test_missing_owner_id_returns_200(self) -> None:
        response = _post(self._http, {'object_type': 'activity', 'aspect_type': 'create', 'object_id': 999})
