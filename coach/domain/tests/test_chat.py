...
class TestChatHistory:
    ...
    def test_has_no_coach_response(self) -> None:
        assert self._history.has_no_assistant_response()
        self._history.add(ChatTurn(role='user', content='hello'))
        assert self._history.has_no_assistant_response()
        self._history.add(ChatTurn(role='assistant', content='hello back'))
        assert not self._history.has_no_assistant_response()
    ...
    def test_old_turns_are_dropped(self) -> None:
        ...
        assert self._history.render() == 'Assistant: Hello back\nUser: How are you?'
